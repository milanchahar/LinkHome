...
import { Menu, X, Home as HomeIcon, Compass, PlusCircle, User, LogOut } from "lucide-react";
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "py-4 bg-dark-900/80 backdrop-blur-xl border-b border-white/10" : "py-6 bg-transparent"
      }`}
    >
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`flex items-center gap-2 text-sm font-medium transition-colors hover:text-brand-500 ${
                location.pathname === link.path ? "text-brand-500" : "text-white/70"
              }`}
            >
              {link.icon}
              {link.name}
